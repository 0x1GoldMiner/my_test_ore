...
pub fn generate_round_report(
    ...
    report.push_str(&format!(
        "│ 总消耗SOL: {:.6}                                  │\n",
        history.total_deployed_sol
    ));
    report.push_str(&format!(
        "│ 总获得SOL: {:.6}                                  │\n",
        history.total_gained_sol
    ));

    let profit_sign = if history.profit_loss_ratio >= 0.0 { "+" } else { "" };
    report.push_str(&format!(
        "│ 盈亏比: {}{:.3} ORE/SOL                                  │\n",
        profit_sign, history.profit_loss_ratio
    ));
    report.push_str(&format!(
        "└─────────────────────────────────────────────────────┘\n"
    ));

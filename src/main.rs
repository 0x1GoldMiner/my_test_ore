    ...
    match strategy {
        MiningStrategy::Threshold {
            ...
            if candidates.len() >= *min_squares {
                // 按部署数量排序，选择最小的 pick_squares 个
                candidates.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
                let selected_candidates: Vec<(usize, f64)> = candidates
                    .into_iter()
                    .take(*pick_squares)
                    .collect();

                // 计算选中格子的部署中位数
                let mut deployment_amounts: Vec<f64> = selected_candidates
                    .iter()
                    .map(|(_, v)| *v)
                    .collect();
                deployment_amounts.sort_by(|a, b| a.partial_cmp(b).unwrap());

                // 中位数 = 第10和11个的平均数（pick_squares >= 20时）
                let median_deployment = if deployment_amounts.len() >= 20 {
                    (deployment_amounts[9] + deployment_amounts[10]) / 2.0
                } else if deployment_amounts.len() > 0 {
                    deployment_amounts[deployment_amounts.len() / 2]
                } else {
                    0.0
                };
